...
def calculate_S_qd_regl_batch_batch_batch(E_q, E_d, device, batch_size: int = 8):
    ...
    # 1) 처음부터 CPU에 두기 (float 캐스팅만)
    E_q = E_q.float().cpu()
    E_d = E_d.float().cpu()

    # 2) 정규화도 CPU에서 수행 (메모리 많이 안 듦, 어차피 한 번만 함)
    E_q_normalized = torch.nn.functional.normalize(E_q, p=2, dim=2)
    E_d_normalized = torch.nn.functional.normalize(E_d, p=2, dim=2)
    ...
    for i in range(0, n_q, batch_size):
        print(f"batch #{batch_cnt} processed...")
        batch_cnt += 1

        # 3) 이때 필요한 q batch만 GPU로 올리기
        q_batch_cpu = E_q_normalized[i : i + batch_size]  # CPU
        q_batch = q_batch_cpu.to(device, non_blocking=True)  # GPU
        bq = q_batch.size(0)

        q_batch_expanded = q_batch.unsqueeze(1)  # (bq, 1, qlen, h)

        for j in range(0, n_d, batch_size):
            d_batch_cpu = E_d_normalized[j : j + batch_size]  # CPU
            d_batch = d_batch_cpu.to(device, non_blocking=True)  # GPU
            bd = d_batch.size(0)
