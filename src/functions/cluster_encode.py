...
def _renew_docs(model, document_batch, device, batch_size=3072, max_length=256):
    torch.cuda.set_device(device)
...
def _renew_data(
    ...
    if renew_q:
        new_q_data = (
            _renew_queries(model, query_batch, device, batch_size, max_length)
            if renew_q
            else {}
        )
    else:
    ...
    if renew_d:
        new_d_data = (
            _renew_docs(model, document_batch, device, batch_size, max_length)
            if renew_d
            else {}
        )
    else:
...
def renew_data(
    ...
    if queries:
        ...
        query_batches = [[None] for _ in range(num_gpus)]
    if documents:
        document_batches = (
        ...
        document_batches = [[None] for _ in range(num_gpus)]
