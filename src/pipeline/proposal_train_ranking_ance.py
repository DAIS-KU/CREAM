    ...
    for epoch in range(num_epochs):
        ...
        for start_idx in range(0, query_cnt, batch_size):
            ...
            for idx in range(start_idx, end_idx):
                query = queries[idx]
                # pos_ids, neg_ids, weight = get_samples_and_weights(
                #     model=model,
                #     query=query,
                #     docs=docs,
                #     clusters=clusters,
                ...
                # )
                pos_ids, neg_ids = get_samples_ance(
                    model,
                    query,
                    docs,
                    clusters,
...
def train(
    ...
    for session_number in range(start_session_number, end_sesison_number):
        ...
        if load_cluster and session_number > 0:
            ...
            with open(f"../data/clusters_{session_number-1}_cls.pkl", "rb") as f:
                clusters = pickle.load(f)
            with open(f"../data/prev_docs_{session_number-1}_cls.pkl", "rb") as f:
                prev_docs = pickle.load(f)
            with open(f"../data/random_vectors_{session_number-1}_cls.pkl", "rb") as f:
        ...
        evaluate(session_number)
        # Evict
        evict_clusters(model, lsh, stream.docs, clusters, ts, required_doc_size)
        # Accumulate
        prev_docs = {**stream.docs, **eval_stream_docs}
        if load_cluster:
            with open(f"../data/clusters_{session_number}_cls.pkl", "wb") as f:
                pickle.dump(clusters, f)
            with open(f"../data/prev_docs_{session_number}_cls.pkl", "wb") as f:
                pickle.dump(prev_docs, f)
            with open(f"../data/random_vectors_{session_number}_cls.pkl", "wb") as f:
...
def evaluate_with_cluster(
    ...
    rankings_path = f"../data/rankings/proposal_{session_number}_with_cluster_cls.txt"
    write_file(rankings_path, result)
    eval_log_path = f"../data/evals/proposal_{session_number}_with_cluster_cls.txt"
...
def evaluate(session_number):
    ...
    rankings_path = f"../data/rankings/{method}_session_{session_number}_cls.txt"
    model_path = f"../data/model/{method}_session_{session_number}_cls.pth"
    ...
    rankings_path = f"../data/rankings/{method}_session_{session_number}_cls.txt"
    write_file(rankings_path, result)
    eval_log_path = f"../data/evals/{method}_{session_number}_cls.txt"
    evaluate_dataset(eval_query_path, rankings_path, eval_doc_count, eval_log_path)
