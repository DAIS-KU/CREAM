...
def train(
    ...
    total_loss_values = []
    loss_values_path = "../data/loss/total_loss_values_proposal_wo_term_msmarco.txt"
    required_doc_size = (
    ...
    for session_number in range(start_session_number, end_sesison_number):
        time_values_path = f"../data/loss/total_time_values_proposal_datasetM_wo_term_{session_number}.txt"
        ts = session_number
        ...
        if session_number != 0:
            ...
            model_path = (
                f"../data/model/proposal_wo_term_msmarco_session_{session_number-1}.pth"
            )
            model.load_state_dict(torch.load(model_path, map_location="cuda:0"))
        model.train()
        new_model_path = (
            f"../data/model/proposal_wo_term_msmarco_session_{session_number}.pth"
        )
        ...
        if (load_cluster or session_number == start_session_number) and (
            ...
            with open(
                f"../data/clusters_wo_term_msmarco_{session_number-1}.pkl", "rb"
            ) as f:
                clusters = pickle.load(f)
                print("Cluster loaded.")
            with open(
                f"../data/prev_docs_wo_term_msmarco_{session_number-1}.pkl", "rb"
            ) as f:
        ...
        with open(f"../data/clusters_wo_term_msmarco_{session_number}.pkl", "wb") as f:
            pickle.dump(clusters, f)
            print("Cluster dumped.")
        with open(f"../data/prev_docs_wo_term_msmarco_{session_number}.pkl", "wb") as f:
...
def _evaluate(session_number):
    method = "proposal_wo_term_msmarco"
