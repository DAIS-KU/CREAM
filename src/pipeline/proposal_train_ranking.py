...
def train(queries, docs, positive_key_name, negative_key_name, method, num_epochs):
    ...
    model.train()
    model_path = f"./model/idea_{method}.pth"
    ...
    for epoch in range(num_epochs):
        ...
        for start_idx in range(0, query_cnt, batch_size):
            ...
            for qid in range(start_idx, end_idx):
                query = queries[qid]
                pos_docs = (
                    [docs[random.sample(query[positive_key_name], 1)[0]]["text"]]
                    if positive_key_name is not None
                    else [doc["text"] for doc in random.sample(list(docs.values()), 1)]
                )
                neg_docs = (
                    [docs[_id]["text"] for _id in query[negative_key_name]]
                    if negative_key_name is not None
                    else [doc["text"] for doc in random.sample(list(docs.values()), 3)]
                )
