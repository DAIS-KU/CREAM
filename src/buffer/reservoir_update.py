        ...
        for i, qid in enumerate(qid_lst):
            ...
            if place_left:
                ...
                if offset == n_doc - 1:
                    filled_idx = list(
                        range(
                            len(buffer.buffer_qid2dids[qid]) - offset,
                            len(buffer.buffer_qid2dids[qid]),
                        )
                    )
            ...
            indices = (
                torch.FloatTensor(len(docids))
                .uniform_(0, buffer.n_seen_so_far[qid])
                .long()
            )  # 从[0, buffer.n_seen_so_far]等概率的采样出query.size(0)个数
