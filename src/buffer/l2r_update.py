...
class L2RUpdate(object):
    ...
    def replace(self, buffer):
        ...
        for qid, candidate_docids in self.candidate_neg_docids.items():
            ...
            if place_left == 0:  # buffer满， 执行替换
                ...
                mem_eval_docids, mem_eval_indices = random_retrieve(
                ...
                qid_lst = [qid]
                mem_eval_grad = self.get_batch_grad(
                    buffer, model_temp, grad_dims, qid_lst, mem_eval_docids_lst
                )  # [参数个数]
                ...
                upsample_mem_docids, upsample_mem_indices = random_retrieve(
                ...
                grad = self.get_each_grad(
                    buffer, model_temp, grad_dims, qid_lst, mem_docids_lst
                )  # [upsample_num, 参数个数]
                mem_sim = cosine_similarity(grad, mem_eval_grad.unsqueeze(0)).squeeze(
                    dim=-1
                )
                ...
                upsample_candidate_docids = random_retrieve(
                    candidate_docids, upsample_num
                )  # list
                new_docids_lst = pos_docid + upsample_candidate_docids
                grad = self.get_each_grad(
                    buffer, model_temp, grad_dims, qid_lst, new_docids_lst
                )  # [upsample_num，参数个数]
                new_sim = cosine_similarity(grad, mem_eval_grad.unsqueeze(0)).squeeze(
                    dim=-1
                )  # [upsample_num]
                new_indices = new_sim.sort(dim=0, descending=False)[
                    1
                ]  # 选相似度最小的一些
                ...
                ] = np.array(upsample_candidate_docids)[new_indices.cpu()].copy()
                ...
                mem_eval_docids = random_retrieve(
                ...
                new_docids_lst = pos_docid + upsample_candidate_docids
                candidate_grad = self.get_each_grad(
                    buffer, model_temp, grad_dims, qid_lst, new_docids_lst
                )  # [upsample_num，参数个数]

                sim = cosine_similarity(
                    candidate_grad, mem_eval_grad.unsqueeze(0)
                ).squeeze(
                    dim=-1
                )  # [candidate个数]
                num_new = min(place_left, len(sim))
                indices = sim.sort(dim=0, descending=False)[1][
                    :num_new
                ]  # 选相似度最小的一些
                buffer.buffer_qid2dids[qid].extend(
                    np.array(upsample_candidate_docids)[indices.cpu()]
                )

    def get_batch_grad(self, buffer, model_temp, grad_dims, qid_lst, docids_lst):
        q_lst = [buffer.qid2query[qid] for qid in qid_lst]
        q_lst = buffer.tokenizer.batch_encode_plus(
            q_lst,
    ...
    def get_each_grad(self, buffer, model_temp, grad_dims, qid_lst, docids_lst):
        ...
        d_lst = buffer.tokenizer.batch_encode_plus(
            d_lst,
            add_special_tokens=True,
            padding="max_length",
            max_length=self.params.p_max_len,
