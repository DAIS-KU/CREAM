...
class MIR_retrieve(object):
    ...
    def retrieve(self, buffer, qid_lst, docids_lst, **kwargs):
        ...
        # retrieve
        docids_lst_from_new = np.array(docids_lst).reshape(len(qid_lst), -1)[
            :, :1
        ]  # pos doc id, [num_q, 1]
        ...
        with torch.no_grad():
            if self.params.compatible:
                ...
                res_post = model_temp.forward(
                    q_lst, doc_lst, identity, doc_emb_from_mem
                )  # 更新后的模型, 返回的emb中已经替换成old emb
            else:
        ...
        indexs = loss.sort(dim=-1, descending=True)[1][
            :, :num_retrieve
        ]  # [num_q, num_retrieve]
    ...
    def get_future_step_parameters(self, model, grad_vector, grad_dims, lr):
        ...
        self.overwrite_grad(
            new_model.parameters, grad_vector, grad_dims
        )  # 用grad_vector设置new_model.parameters的梯度
        with torch.no_grad():
    ...
    def cal_loss(self, model_output):
        ...
        scores = torch.matmul(q_reps.unsqueeze(dim=1), p_reps.transpose(1, 2)).squeeze(
            dim=1
        )  # [num_q, n_psg]
        ...
        target = torch.tensor(
            [0] * scores.size(0), device=scores.device, dtype=torch.long
        )  # [num_q*(n_psg-1)]
        loss = F.cross_entropy(scores, target, reduction="none").reshape(
            num_q, n_psg - 1
        )  # [num_q, n_psg-1]
        return loss
