...
from clusters import Stream, renew_data
...
def train(
    ...
    for session_number in range(start_session_number, end_sesison_number):
        ...
        # Evict
        evict_clusters(model, stream.docs, clusters, ts, required_doc_size)
...
def evaluate_with_cluster(
    ...
    rankings_path = (
        f"../data/rankings/proposal_wo_term_pre_{session_number}_with_cluster.txt"
    )
    write_file(rankings_path, result)
    eval_log_path = (
        f"../data/evals/proposal_wo_term_pre_{session_number}_with_cluster.txt"
    )
...
def evaluate(session_number, model_path):
    ...
    rankings_path = (
        f"../data/rankings/proposal_wo_term_session_pre_{session_number}.txt"
    )
    write_file(rankings_path, result)
    eval_log_path = f"../data/evals/proposal_wo_term_session_pre_{session_number}.txt"
    evaluate_dataset(eval_query_path, rankings_path, eval_doc_count, eval_log_path)
